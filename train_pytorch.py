...
model = model_rnn.MyModel().to(device)
...
for epoch in range(epochs):
    ...
    for i in range(int(driving_data.num_images / batch_size*0.8)):
        ...
        if i%1==0:
            print("Epoch: %d, step: %d Train Loss: %g" % (epoch,i, loss))
    for i in range(int(driving_data.num_images / batch_size*0.2)):
        ...
        if i % batch_size == 0:
            if not os.path.exists(LOGDIR):
                os.makedirs(LOGDIR)
            checkpoint_path = os.path.join(LOGDIR, "model_rnn12.ckpt")
            torch.save(model.state_dict(), checkpoint_path)
    print("Model saved in file: %s" % checkpoint_path)
    
    loss_train_average=loss_train_sum/(driving_data.num_images / batch_size*0.8)
